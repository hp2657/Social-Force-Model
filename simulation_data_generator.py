...
if __name__ == "__main__":
    batch = 5  # Number of batches

    for i in range(batch):
        print(f"Running batch {i + 1}/{batch}...")
        ...
        # Save state_record for current batch
        save_record = s.state_record[:1000]  # Adjust truncation length
        np.save(f"state_record_batch_{i + 1}.npy", save_record)  # Save data to file

        # Create animation for current batch
        sa = SceneAnimation(save_record, obs, f"animation_batch_{i + 1}")
        sa.animate()

    print("All batches completed. Animations and state records have been saved.")
