...
# Load data to device (fixed to CPU)
...
def compute_expected_acceleration(positions, velocities, walls, social_force_threshold, wall_force_threshold):
    ...
    for wall in walls:
        wall_start = torch.tensor([wall[0], wall[2]], dtype=torch.float32)
        wall_end = torch.tensor([wall[1], wall[3]], dtype=torch.float32)
...
def create_animation(data, walls, interval=50):
    """
    Create trajectory animation to display pedestrian trajectories and obstacles.
    Args:
        data (numpy.ndarray): Pedestrian trajectory data, shape [num_pedestrians, sequence_length, feature_dim]
        walls (list): Obstacles defined as [x1, y1, x2, y2]
        interval (int): Time interval between frames (milliseconds)
    """
    ...
    # Update function
    def update(frame):
        positions = data[:, frame, :]  # Current frame's pedestrian positions
