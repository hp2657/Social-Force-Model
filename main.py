...
'''
Based on Simulating Dynamical Features of Escape Panic
Helbing, Dirk , I. Farkas , and T. Vicsek . "Simulating Dynamical Features of Escape Panic." 
Nature 407.6803(2000):487-90.
'''
...
'''
departure_region: list with x_min, x_max, y_min, y_max, initial_speed_x, initial_speed_y
initial_speed_x, initial_speed_y should be a unit vector
'''
departure_region = {
    'a': [-3.5,3,-3.5,3,0,0]
    }
...
'''
destination_region: list with x_min, x_max, y_min, y_max
'''
...
'''
storage_position: After the pedestrian moves to the destination, we will move the pedestrian to a location
x and y coordinates of position
'''

storage_position = np.array([6, 6])
...
'''
obs: each line means an obstacle, x_min, x_max, y_min, y_max
'''
...
'''
initial_vel: Scalar quantity, the initial speed of pedestrain
max_vel: Scalar quantity, the max speed of pedestrain
Pedestrian velocity follows normal distribution
'''
...
'''
mean_weight: average weight of pedestrain
std_weight: Standard deviation of pedestrian weight
Pedestrian weight follows normal distribution
'''
...
'''
num_of_people_keep, release_time_interval
The pedestrians are released one by one until the number reaches num_of_people_keep. 
The interval at which pedestrians are released is release_time_interval. 
After the number of pedestrians reaches num_of_people_keep, 
a new pedestrian will be released for each pedestrian that reaches the end.
'''

release_system_parameter = {
    'num_of_people_keep': 50,
    'release_time_interval': 0
    }


'''
pedestrain_generator: Generate random pedestrian data
'''
...
class Simulator:
    ...
    '''
    compute_forces: return the accelaration of pedestrain
    '''
...
'''
PedState: Calculate the speed, acceleration, and movement of each step of the pedestrian
'''
...
'''
Force: Compute force and acceleration
'''
...
'''
SceneAnimation: Output animation of pedestrian movement
'''
...
'''
TrajectoryShow: Output picture of pedestrian trajectory
'''
...
if __name__ == "__main__":
    ...
    '''
    SceneAnimation parameter: 'animation': the file name
    '''
    ...
    '''
    point_index: Select the pedestrian number you want to output the track of
    
    TrajectoryShow parameter: 'trajectory': file name
    '''
...
def Truncation(state_record):
    last_item = state_record[-1]
    index = np.where(last_item[:,8]==0)[0][0]
    return [item[0:index,:] for item in state_record]


#save_record = Truncation(s.state_record)
save_record = s.state_record[0:1000]
...
def data_write_csv(file_name, datas):
        ...
        np.save(file_name, datas)

print("List of arrays saved to array_list.npy")

data_write_csv('state record', save_record)
